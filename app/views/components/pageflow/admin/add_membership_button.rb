  ...
  module Admin
    class AddMembershipButton < ViewComponent
      ...
      def entry_membership_possible_for_creator_and(user, parent)
        case parent
        when User
          PotentialMemberships.new(user).entries_for_user(parent).any?
        when Entry
          PotentialMemberships.new(user).users_for_entry(parent).any?
        end
      end
      ...
      def account_membership_possible_for_creator_and(user, parent)
        case parent
        when User
          PotentialMemberships.new(user).accounts_for_user(parent).any?
        when Account
          PotentialMemberships.new(user).users_for_account(parent).any?
        end
      end
